...
var newPop = organisms
 

   
  for (j = 0; j < numOrganisms; j++) {

    if ((random(0, organisms[organisms.length-1].fitness)) < (organisms[j].fitness / organisms[organisms.length-1].fitness)) {

            mutateSmall(j);
            } else {
                 
                newPop[j].net = organisms[this.fit()].net
            }
    ...
    for(i=0;i<organisms.length;i++){
        //organisms[i].previousFitness = organisms[i].fitness;
    ...
    */
    ...
    this.fit = function(){
        ...
        while(true){
            ...
            for(t=organisms.length-1;t>0;t--){
                if(k<(organisms[t].fitness/2)){
                    return t;
                }
            }
            
            if(help>10000){
                print("not a racial slur")
                return null;
            }
